...
export const FoodGeneration = () => {
  ...
  const generateImageAndExtract = async () => {
    ...
    try {
      ...
      const infoArray = extractData.result.split(",").map((s: string) => s.trim());
  ...
  return (
    <main className="container max-w-3xl p-4 mx-auto">
      ...
      <div className="space-y-4">
        ...
        />
        <Button onClick={generateImageAndExtract} disabled={isLoading || !prompt.trim()} className="w-full">
          {isLoading ? (
        ...
        {error && <div className="p-3 text-red-500 rounded bg-red-50 border border-red-200">{error}</div>}
            ...
            <div className="space-y-6">
              ...
              {resultImage && (
                <div className="overflow-hidden border rounded-xl shadow-lg">
                  <img src={resultImage} alt="Generated food" className="w-full h-auto transition-all hover:scale-[1.02]" />
                </div>
              )}
            </div>
          )}
